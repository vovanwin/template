...
// AuthRefreshPostParams is parameters of POST /auth/refresh operation.
type AuthRefreshPostParams struct {
	// Уникальный идентификатор запроса (UUID) отправляется с
	// фронтенда для идентификации каждого запроса и
	// логирования соответствующих логов, относящихся к
	// данному запросу.
...
func unpackAuthRefreshPostParams(packed middleware.Parameters) (params AuthRefreshPostParams) {
	{
		key := middleware.ParameterKey{
			Name: "X-Request-Id",
			In:   "header",
...
func decodeAuthRefreshPostParams(args [0]string, argsEscaped bool, r *http.Request) (params AuthRefreshPostParams, _ error) {
	h := uri.NewHeaderDecoder(r.Header)
	// Decode header: X-Request-Id.
	if err := func() error {
		cfg := uri.HeaderParameterDecodingConfig{
